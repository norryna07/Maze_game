...
class MazeActions {
    ...
    static bool inside_cell(std::pair<int, int> &pair, int lin, int col);
    template <class T> static T getRandomElement(std::set<T> set1);
    template <class T> static  std::pair<T, T> getTwoRandomElements(std::vector<T> v);
